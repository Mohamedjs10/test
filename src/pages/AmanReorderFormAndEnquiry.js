...
const AmanReorderFormAndEnquiry = () => {
  //* redux
  ...
  //* refs & states *****************************************
  const billingAccountRef = useRef(initialBillingAccount); // also set as defaultValue in amanInputs // as no state handling it here
  const billingAmountRef = useRef(initialBillingAmount);
  const [topUpAmtount, setTopUpAmtount] = useState(initialTopupAmount); // balance (top up) amount => 7    [use to fetch enquiry for all services]
  const [chargeAmtount, setChargeAmtount] = useState(initialChargeAmount); // payment (charge) amount => 10 [use to fetch enquiry only for (id === 59 + 63)]
  const [isChecked, setIsChecked] = useState(isInQuickPay(currentService.code));
  const [aliasName, setAliasName] = useState("");
  const [alertMessage, setAlertMessage] = useState("");
  const [errorAlert, setErrorAlert] = useState(false); // no need, should be removed
  ...
  useEffect(() => {
    ...
    if (currentService?.payment_type === "PAY") {
  ...
  return (
    <div className="aman-history" dir={t("layout.dir")}>
      ...
      {/* enquiryResult  =>  if no-amount-needed service ("PAY": aman) ("POSTPAID": khales) --------------- */}
      {enquiryResult ? (
        <div className="container">
...
export default AmanReorderFormAndEnquiry;
